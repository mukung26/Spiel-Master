...
import { saveEditorState, getEditorState } from '../services/editorStorage';
...
export const EditorView: React.FC = () => {
  ...
  useEffect(() => {
    const saveState = async () => {
      // If no image, we don't auto-save here (deletion handles clearing)
      if (!bgImage) return;
      ...
      saveEditorState(base64, objects);
    };
  ...
  useEffect(() => {
    ...
    if (bgImage) {
        ...
        // Placeholder background - Update to match the new dark mode theme if needed
  ...
  const handleDeleteImage = async () => {
    // Instant Reset - No confirmation dialog
    setBgImage(null);
    setObjects([]);
    setHistory([[]]);
    setHistoryIndex(0);
    ...
    // Explicitly clear storage to prevent reload restoration
    try {
        await saveEditorState(null, []);
    } catch (e) {
        console.error("Failed to clear editor storage", e);
    }
  };
    ...
    <div className="h-full flex flex-col bg-[#f0f2f5] dark:bg-[#202124]">
      ...
      <div className="bg-white dark:bg-[#1e1e1e] p-2 border-t border-gray-200 dark:border-gray-800 text-center">
         <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest flex items-center justify-center gap-2">
            <Scissors size={12} />
            Paste Image (Ctrl+V) &bull; Undo (Ctrl+Z) &bull; Redo (Ctrl+Y) &bull; Auto-saved
         </p>
      </div>
    </div>
  );
