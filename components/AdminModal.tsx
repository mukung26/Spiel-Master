...
const COLORS = [
  ...
  { label: 'Red', value: 'bg-red-50 dark:bg-[#450a0a]' },
  { label: 'Orange', value: 'bg-orange-50 dark:bg-[#431407]' },
  { label: 'Yellow', value: 'bg-yellow-50 dark:bg-[#422006]' },
  { label: 'Green', value: 'bg-green-50 dark:bg-[#052e16]' },
  { label: 'Teal', value: 'bg-teal-50 dark:bg-[#042f2e]' },
  { label: 'Blue', value: 'bg-blue-50 dark:bg-[#172554]' },
  { label: 'Purple', value: 'bg-purple-50 dark:bg-[#3b0764]' },
  { label: 'Pink', value: 'bg-pink-50 dark:bg-[#500724]' },
  { label: 'Gray', value: 'bg-gray-50 dark:bg-[#1f2937]' },
];

export const AdminModal: React.FC<AdminModalProps> = ({ 
  categories, onClose, onSubmit, initialData, onPushInitialData, onRepairDatabase, currentSpiels = [], isAdmin = false
}) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        color: initialData.color || 'bg-white dark:bg-[#202124]'
      });
    }
  }, [initialData]);
        ...
        <div className="px-8 pt-8 border-b border-black/5 dark:border-white/10">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3 flex-1 mr-8">
              <Database className="text-orange-500 flex-shrink-0" size={24} />
              <input 
                type="text" 
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                placeholder="ENTER TITLE"
                className="w-full bg-transparent border-none text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter placeholder-gray-300 dark:placeholder-gray-700 focus:ring-0 p-0"
                required
