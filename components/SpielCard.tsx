...
interface SpielCardProps {
  ...
  isAdmin?: boolean;
  onClick?: () => void;
  onEdit?: () => void; // Kept for interface compatibility but pencil removed from UI
  onDelete?: () => void;
  onRestore?: () => void;
  onPin?: () => void;
  onExpand?: () => void;
...
export const SpielCard: React.FC<SpielCardProps> = ({ 
  spiel, onCopy, isAdmin, onClick, onEdit, onDelete, onRestore, onPin, onExpand, isTrashView = false, onViewImage, isEditMode = false
}) => {
  ...
  // Text color logic: If urgent or has custom color (not default), use white text in dark mode for contrast against solid colors
  const isUrgentDark = isUrgent && !isTrashView;
  const hasCustomColor = spiel.color && !spiel.color.includes('bg-white');
  
  const textColorClass = (isUrgentDark || hasCustomColor) ? 'text-gray-900 dark:text-white' : 'text-gray-900 dark:text-gray-100';
  const subTextColorClass = (isUrgentDark || hasCustomColor) ? 'text-gray-600 dark:text-white/80' : 'text-gray-600 dark:text-gray-300';
  const borderColorClass = (isUrgentDark || hasCustomColor) ? 'border-gray-200 dark:border-white/20' : 'border-gray-200 dark:border-gray-700';
  ...
  return (
    <div 
      ...
      `}
      onClick={!isTrashView ? (onClick || onExpand) : undefined}
    >
      ...
      <div className="p-4 flex flex-col gap-2 flex-grow">
                ...
                {/* Pin Indicator on Card */}
                 {spiel.isPinned && !isTrashView && (
                    <div className="absolute top-0 right-0 -mt-1 -mr-1">
                         <Pin size={12} className={hasCustomColor ? "text-gray-900 dark:text-white fill-current" : "text-gray-900 dark:text-gray-100 fill-current"} />
                    </div>
                 )}
            </div>
         </div>
         ...
         {/* Footer Controls - Always Visible now */}
         <div className={`flex items-center justify-between pt-3 mt-auto`}>
             {/* Copy Chips - Always visible */}
             <div className="flex gap-1.5">
                {spiel.english && (
                   <button 
                     onClick={(e) => { e.stopPropagation(); copyToClipboard(spiel.english, 'EN'); }}
                     className={`px-2 py-0.5 rounded text-[9px] font-black border opacity-80 hover:opacity-100 transition-opacity ${hasCustomColor ? 'border-gray-300 text-gray-500 dark:border-white/40 dark:text-white' : 'border-gray-300 text-gray-500 dark:border-gray-600 dark:text-gray-400'}`}
                   >
                     EN
                   </button>
                )}
             ...
             <div className="flex gap-1">
                {!isTrashView ? (
                  <>
                     {onPin && (
                        <button onClick={(e) => {e.stopPropagation(); onPin()}} className={`p-1.5 rounded-full hover:bg-black/10 dark:hover:bg-white/10 ${spiel.isPinned ? (hasCustomColor ? 'text-gray-900 dark:text-white' : 'text-gray-900 dark:text-gray-100') : 'text-gray-400'}`}>
                           <Pin size={14} className={spiel.isPinned ? "fill-current" : ""} />
                        </button>
                     )}
                     {/* Pencil Removed as per request, editing is now via clicking the card */}
